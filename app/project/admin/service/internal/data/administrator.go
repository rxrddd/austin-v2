...
func (rp administratorRepo) GetAdministrator(ctx context.Context, id int64) (*biz.Administrator, error) {
	result, err, _ := rp.sg.Do(fmt.Sprintf("find_user_by_id_%s", id), func() (interface{}, error) {
		user, err := rp.data.administratorClient.GetAdministrator(ctx, &v1.GetAdministratorRequest{
			Id: id,
		})
		if err != nil {
			return nil, err
...
func (rp administratorRepo) VerifyPassword(ctx context.Context, id int64, password string) error {
	reply, err := rp.data.administratorClient.VerifyAdministratorPassword(ctx, &administratorClientV1.VerifyAdministratorPasswordRequest{
		Id:       id,
		Password: password,
	})
	if err != nil {
