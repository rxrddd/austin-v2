...
func (a AuthorizationRepo) GetRoleMenuBtn(ctx context.Context, roleId int64, roleName string, menuId int64) (response []*biz.MenuBtn, err error) {
	...
	// 查询角色拥有哪些菜单按钮
	var roleMenuBtn []entity2.RoleMenuBtn
	conn := a.data.db.Model(entity2.RoleMenuBtn{})
	if menuId != 0 {
	...
	btnResponse := []*entity2.MenuBtn{}
	if len(btnIds) == 0 {
		return []*biz.MenuBtn{}, nil
	}
	a.data.db.Model(&entity2.MenuBtn{}).Where("id IN (?)", btnIds).Find(&btnResponse)
...
func (a AuthorizationRepo) SetRoleMenuBtn(ctx context.Context, roleId int64, menuId int64, btnIds []int64) error {
	tx := a.data.db.Begin()
	// 先删除数据
	err := tx.Where("role_id = ? AND menu_id = ?", roleId, menuId).Delete(&entity2.RoleMenuBtn{}).Error
	if err != nil {
		tx.Rollback()
		return kerrors.InternalServer(errResponse.ReasonSystemError, err.Error())
	}
	...
	// 批量插入数据
	roleMenuBtn := []entity2.RoleMenuBtn{}
	for _, v := range btnIds {
		roleMenuBtn = append(roleMenuBtn, entity2.RoleMenuBtn{
			RoleId: roleId,
			MenuId: menuId,
			BtnId:  v,
		})
