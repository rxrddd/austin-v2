...
import (
	...
	"austin-v2/pkg/utils/convertHelper"
...
func (a AuthorizationRepo) UpdateMenu(ctx context.Context, reqData *biz.Menu) (*biz.Menu, error) {
	...
	var menu entity2.Menu
	a.data.db.Model(entity2.Menu{}).Where("id = ?", reqData.Id).First(&menu)
	menu.Id = reqData.Id
	...
	// 关联数据更新
	tx := a.data.db.Begin()
	err := tx.Model(entity2.Menu{}).Where("id = ?", menu.Id).Session(&gorm.Session{FullSaveAssociations: true}).Save(&menu).Error
	if err != nil {
		tx.Rollback()
		return &biz.Menu{}, kerrors.InternalServer(errResponse.ReasonSystemError, err.Error())
	}
...
func (a AuthorizationRepo) DeleteMenu(ctx context.Context, id int64) error {
	...
	a.data.db.Model(entity2.RoleMenu{}).Where("menu_id = ?", id).First(&roleMenu)
	if roleMenu.Id != 0 {
